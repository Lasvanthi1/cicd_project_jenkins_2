                  ...
                  // def dockerbuild = 'docker-compose build'
                    ...
                    sshagent(['sshkeypair']) {
                       ...
                       //  sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.36.171 ${dockerbuild}"
                         sh "ssh -o StrictHostKeyChecking=no ubuntu@3.109.207.182 wget https://raw.githubusercontent.com/Lasvanthi1/cicd_project_jenkins_2/master/docker-compose.yaml"
                         sh "ssh -o StrictHostKeyChecking=no ubuntu@3.109.207.182 chmod 777 docker-compose.yaml"
                         sh "ssh -o StrictHostKeyChecking=no ubuntu@3.109.207.182 ${dockerCmd}"
                    }
                }
            }
        }
